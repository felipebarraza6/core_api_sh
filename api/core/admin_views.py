...
from django.db.models import Avg, Count, IntegerField, Max, Prefetch, Q, Sum
...
from api.telemetry.validators.telemetry_validator import (
    ...
    validate_flow_impossible,
    validate_level_impossible,
)
...
def admin_dashboard_view(request):
    ...
    try:
        ...
        logging.info(
            f"🔍 Valores recibidos - project_id: {project_id} (tipo: {type(project_id)}), point_id: {point_id} (tipo: {type(point_id)})"
        )
        ...
        if project_id:
            try:
                ...
                logging.warning(
                    f"❌ No se pudo convertir project_id '{project_id}' a entero"
                )
                project_id = None
        ...
        if point_id:
            try:
                ...
                logging.warning(
                    f"❌ No se pudo convertir point_id '{point_id}' a entero"
                )
                point_id = None
        ...
        period_days = (
            period_days_map.get(veracidad_periodo, 90)
        ...
        if point_id:
            try:
                ...
                point_filter = Q(point=selected_point)
        ...
        obras_with_telemetry = (
            ProfileDataConfigCatchment.objects.filter(
                point_catchment__in=obras_points_ids, is_telemetry=True
            )
            .values("point_catchment")
            .distinct()
            .count()
        )

        pct_conectados = (
            (obras_with_telemetry / num_obras * 100) if num_obras > 0 else 0
        )
        ...
        desconectados_count = (
            num_obras - pct_conectados_count if num_obras >= pct_conectados_count else 0
        )
        ...
        for point in obras_points_list:
            ...
            if dga_profile:
                ...
                if "SIN_ESTANDAR" not in dga_standards_count:
                    dga_standards_count["SIN_ESTANDAR"] = {
                        "total": 0,
                        "connected": 0,
                        "disconnected": 0,
                    }
        ...
        latest_record_ids_subquery = (
            TelemetryRecord.objects.filter(point_id=OuterRef("id"))
            .order_by("-timestamp")
            .values("id")[:1]
        )

        # Anotar puntos con el ID de su último registro
        obras_points_annotated = (
            CatchmentPoint.objects.filter(id__in=obras_points_ids)
        ...
        latest_record_ids_list = [
            p.latest_record_id for p in obras_points_annotated if p.latest_record_id
        ]
        all_latest_records = TelemetryRecord.objects.filter(
            id__in=latest_record_ids_list
        ).select_related("point", "point__project")

        # Crear diccionario para acceso O(1) sin queries adicionales
        records_by_point_id = {r.point_id: r for r in all_latest_records}
        ...
        for point in obras_points_annotated:
            ...
            has_caudal = bool({"flow", "caudal"} & set(variable_types))
            has_caudal_promedio = bool(
                {"caudal_promedio", "flow_avg", "avg_flow"} & set(variable_types)
            )
            ...
            if has_caudal_promedio and d6_safe > 0:
                try:
                    from api.telemetry.utils.flow_display import (
                        get_interaction_flow_display_data,
                    )

                    flow_value = get_interaction_flow_display_data(last_record).get(
                        "value", flow_value
                    )
                except Exception:
                    flow_value = last_record.flow or 0.0
            ...
            if flow_value_safe_veracidad_final > 0 and diametro:
                ...
                if flow_value_safe_veracidad_final > caudal_probable:
                    ...
                    pct_exceso = (
                        (exceso / caudal_probable * 100) if caudal_probable > 0 else 0
                    )
                    ...
                    proyecto_nombre = (
                        point.project.name if point.project else "Sin proyecto"
                    )

                    # Obtener horario de captura
                    horario_captura = (
                        last_record.timestamp.strftime("%d/%m/%Y %H:%M")
                    ...
                    punto_info = {
                        "nombre": (
                            str(point.title) if point and point.title else "Sin nombre"
                        ),
                        "punto": (
                            str(point.title) if point and point.title else "Sin nombre"
                        ),  # Para compatibilidad con template unificado
                        "proyecto": proyecto_nombre,
                        "codigo_obra": codigo_obra,
                        "horario": horario_captura,
                        "fecha": last_record.timestamp,  # Agregar fecha para compatibilidad
                        "caudal_medido": round(flow_value_safe_veracidad_final, 2),
                        "caudal_real": round(
                            flow_value_safe_veracidad_final, 2
                        ),  # Para compatibilidad
                        "caudal_probable": round(caudal_probable, 2),
                        "exceso": round(exceso, 2),
                        "pct_exceso": round(pct_exceso, 1),
                        "variables": (
                            variable_types if variable_types else []
                    ...
                    if point.id not in veracidad_dict:
                        ...
                        existing_exceso = safe_float(
                            veracidad_dict[point.id].get("exceso", 0)
                        )
        ...
        for punto_info in veracidad_dict.values():
            ...
            punto_info["priority"] = (
                2  # Prioridad: Caudal Imposible (3) > Exceso Caudal (2) > Nivel Imposible (1)
            )
            punto_info["fecha"] = (
                None  # Se obtendrá del último registro si es necesario
            )
        ...
        if use_historical_veracidad:
            try:
                from api.core.utils.veracidad_historica import (
                    calculate_historical_veracidad,
                )

                # Calcular veracidad histórica para el período seleccionado
                historical_veracidad = calculate_historical_veracidad(
                    obras_points_list=obras_points_list, period_days=period_days
                )
                ...
                veracidad_pasan_probable = historical_veracidad[
                    "pct_veracidad_pasan_probable"
                ]
                ...
                logging.error(
                    f"Error calculando veracidad histórica: {e}", exc_info=True
                )
                ...
                pct_veracidad = (
                    (
                        (points_with_d5 - points_flow_above_probable)
                        / points_with_d5
                        * 100
                    )
                    if points_with_d5 > 0
            ...
            pct_veracidad = (
                ((points_with_d5 - points_flow_above_probable) / points_with_d5 * 100)
                if points_with_d5 > 0
                else 0
            )
            ...
            veracidad_total_puntos = points_with_flow
            veracidad_puntos_con_d5 = points_with_d5
            veracidad_pasan_probable = (
                points_flow_above_probable  # Puntos que SÍ pasan el caudal probable
            )
        ...
        latest_records_qs = TelemetryRecord.objects.filter(
            point_id=OuterRef("point_id")
        ).order_by("-timestamp")

        # Opción más simple: traer los records directamente usando ID__in
        # (TelemetryRecord puede ser grande, así que filtramos por los puntos visibles)
        # Para evitar N+1, usamos un diccionario

        # Subquery para obtener el ID del último registro de cada punto
        latest_ids = (
            TelemetryRecord.objects.filter(point__in=all_visible_ids)
            .order_by("point", "-timestamp")
            .distinct("point")
        ...
        latest_records_map = {
            r.point_id: r for r in TelemetryRecord.objects.filter(id__in=latest_ids)
        }
        ...
        errores_dict = (
            {}
        )  # Diccionario para evitar duplicados: key=point.id, value=dict con info del error
            ...
            if flow_value_safe_error > 0:
                ...
                if d5_safe_error > 0:
                    ...
                    es_imposible, mensaje = validate_flow_impossible(
                        flow_value_safe_error, d5_safe_error
                    )
                    if es_imposible:
                        registro_errores += 1
                        error_tipo = "Caudal Imposible"
                        # Calcular caudal probable para mostrar el exceso
                        caudal_probable_calc = calculate_probable_flow_by_velocity(
                            d5_safe_error, 2.5
                        )
                        ...
                        if (
                            caudal_excedido is not None
                            and caudal_probable_calc is not None
                            and caudal_probable_calc > 0
                        ):
                            pct_exceso_calc = round(
                                (caudal_excedido / caudal_probable_calc * 100), 1
                            )
                        ...
                        error_info = {
                            ...
                            "caudal_probable": (
                                round(caudal_probable_calc, 2)
                                if caudal_probable_calc
                                else None
                            ),
                            "exceso": (
                                round(caudal_excedido, 2) if caudal_excedido else None
                            ),
                            "pct_exceso": pct_exceso_calc,  # Agregar porcentaje de exceso
                            "variables": (
                                variable_types_error if variable_types_error else []
                            ),  # Asegurar que siempre sea una lista
                            "pulses": last_record.pulses or 0,
                            "nivel": last_record.nivel or None,
                            "tipo": "caudal",
                            ...
                            "proyecto": (
                                str(point.project.name)
                                if point and point.project and point.project.name
                                else None
                            ),  # Agregar proyecto
                        ...
                        if point.id not in errores_dict:
                            ...
                            existing_priority = errores_dict[point.id].get(
                                "priority", 0
                            )
                    ...
                    if nivel_value > 0:
                        ...
                        if es_imposible:
                            ...
                            if point.id not in errores_dict:
                                ...
                                if (
                                    error_priority.get(error_tipo, 0)
                                    > existing_priority
                                ):
                                    errores_dict[point.id] = error_info
                except (ValueError, TypeError):
        ...
        errores_y_excesos_combinados.sort(
        ...
        records_in_dga_queue = TelemetryRecord.objects.filter(
            combined_filter, send_dga=True
        ).count()

        # Total de registros para calcular porcentaje
        total_registros_cola_dga = TelemetryRecord.objects.filter(
            combined_filter
        ).count()
        pct_cola_dga = (
            (records_in_dga_queue / total_registros_cola_dga * 100)
            if total_registros_cola_dga > 0
            else 0
        )
        ...
        try:
            ...
            try:
                ...
                logging.warning(
                    f"Página inválida solicitada (no es número): {page_number}"
                )
                page_number = 1
            ...
            if isinstance(e, (PageNotAnInteger, EmptyPage)):
                ...
                logging.error(
                    f"Error inesperado en paginación: {str(e)}", exc_info=True
                )
        ...
        error_interactions = (
            TelemetryRecord.objects.filter(combined_filter, is_error=True)
            .annotate(
                _days_nc=Cast("metadata__days_not_conection", IntegerField()),
                _days_nc_alt=Cast("metadata__days_not_connection", IntegerField()),
            )
            .exclude(Q(_days_nc__gt=0) | Q(_days_nc_alt__gt=0))
        )

        # Obtener puntos con errores (sin días de desconexión)
        error_points = error_interactions.values_list("point", flat=True).distinct()
        ...
        project_points = []
        if project_id:
            project_points = CatchmentPoint.objects.filter(
                project=selected_project
            ).order_by("title")
        else:
            project_points = CatchmentPoint.objects.all().order_by("title")[
                :50
            ]  # Limitar si no hay filtro
        ...
        if variable_filter == "caudal":
            ...
            visible_points = visible_points.filter(
                variables__internal_code__in=["flow", "caudal"]
            ).distinct()
        elif variable_filter == "promedio":
            visible_points = visible_points.filter(
                variables__internal_code__in=["caudal_promedio", "flow_avg", "avg_flow"]
            ).distinct()
        # END FILTER LOGIC
        ...
        for point_id in visible_points_with_code:
            ...
            last_record = (
                TelemetryRecord.objects.filter(
                    point_id=point_id, timestamp__gte=last_24_hours
                )
                .select_related("point", "point__project")
        ...
        if recent_interactions:
            ...
            recent_interactions = (
                TelemetryRecord.objects.filter(id__in=record_ids)
                .select_related("point", "point__project")
                .prefetch_related(
                    Prefetch(
                        "point__variables",
                    ...
                    Prefetch(
                        ...
                        queryset=ProfileDataConfigCatchment.objects.only(
                            "point_catchment_id", "d6", "is_telemetry", "d3"
                        ),
                    ),
                    Prefetch(
                        "point__dga_data_config_profiles",
        ...
        recent_interactions.sort(
            key=lambda x: (
                not (
                    x.days_not_conection and x.days_not_conection > 0
                ),  # False (desconectados) primero
                -(
                    x.timestamp.timestamp()
                    if x and x.timestamp
                    else timezone.now().timestamp()
                ),  # Luego por fecha descendente (negativo para reverse)
            )
        )
        ...
        point_ids_voucher = [i.point_id for i in recent_interactions]
        ...
        if unique_point_ids_voucher:
            try:
                ...
                latest_vouchers_qs = (
                    TelemetryRecord.objects.filter(
                        point_id__in=unique_point_ids_voucher, n_voucher__isnull=False
                    )
                    .exclude(n_voucher="")
                ...
                latest_vouchers_map = {v.point_id: v for v in latest_vouchers_qs}
            except Exception as e:
        ...
        for interaction in recent_interactions:
            ...
            if (
                hasattr(point, "_prefetched_objects_cache")
                and "variables" in point._prefetched_objects_cache
            ):
            ...
            variable_types = sorted(
                {
                    v.internal_code
                    for v in variables
                    if getattr(v, "internal_code", None)
                }
            )
            ...
            if (
                hasattr(point, "_prefetched_objects_cache")
                and "data_config_profiles" in point._prefetched_objects_cache
            ):
            ...
            if (
                hasattr(point, "_prefetched_objects_cache")
                and "dga_data_config_profiles" in point._prefetched_objects_cache
            ):
                dga_profiles = point._prefetched_objects_cache[
            ...
            if has_caudal_promedio and has_d6:
                try:
                    ...
                    flow_value = get_interaction_flow_display_data(interaction).get(
                        "value", flow_value
                    )
                except Exception:
                    flow_value = interaction.flow or 0.0
            ...
            if caudal_autorizado_safe > 0 and flow_value_safe_enhanced > 0:
                diferencia_caudal_dga = (
                    caudal_autorizado_safe - flow_value_safe_enhanced
                )
                # Porcentaje usado: cuánto del autorizado se está usando
                pct_usado_caudal_dga = (
                    (flow_value_safe_enhanced / caudal_autorizado_safe * 100)
                    if caudal_autorizado_safe > 0
                    else 0
                )
            ...
            if (
                profile
                and profile.d5
                and float(profile.d5) > 0
                and flow_value_safe_enhanced > 0
            ):
            ...
            if water_table_safe > 0 and d3_posicionamiento_safe > 0:
                if water_table_safe > d3_posicionamiento_safe:
                    ...
                    nivel_veracidad_msg = (
                        "ERROR CRÍTICO: Nivel freático superior al posicionamiento"
                    )
                else:
                    # Calcular diferencia como porcentaje
                    diferencia_nivel = d3_posicionamiento_safe - water_table_safe
                    pct_diferencia = (
                        (diferencia_nivel / d3_posicionamiento_safe * 100)
                        if d3_posicionamiento_safe > 0
                        else 0
                    )
            ...
            try:
                if current_pulses is not None:
                    last_different_pulses_record = (
                        TelemetryRecord.objects.filter(
                            point_id=point.id, timestamp__lt=interaction.timestamp
                        )
                        .exclude(pulses=current_pulses)
                        .order_by("-timestamp")
                        .first()
                    )
                else:
                    last_different_pulses_record = (
                        TelemetryRecord.objects.filter(
                            point_id=point.id,
                            timestamp__lt=interaction.timestamp,
                ...
                if (
                    last_different_pulses_record
                    and last_different_pulses_record.timestamp
                ):
                    time_diff = (
                        interaction.timestamp - last_different_pulses_record.timestamp
                    )
                    pulses_days_unchanged = time_diff.days
                    pulses_hours_unchanged = time_diff.seconds // 3600
                else:
                    first_record = (
                        TelemetryRecord.objects.filter(point_id=point.id)
                        .order_by("timestamp")
                        .first()
                    )
                    ...
                    if (
                        first_record
                        and first_record.timestamp
                        and interaction.timestamp
                    ):
                        time_diff = interaction.timestamp - first_record.timestamp
                        pulses_days_unchanged = time_diff.days
                        pulses_hours_unchanged = time_diff.seconds // 3600
            except Exception as e:
                pass
            ...
            try:
                if current_nivel is not None:
                    last_different_nivel_record = (
                        TelemetryRecord.objects.filter(
                            point_id=point.id, timestamp__lt=interaction.timestamp
                        )
                        .exclude(nivel=current_nivel)
                        .order_by("-timestamp")
                        .first()
                    )
                else:
                ...
                if (
                    last_different_nivel_record
                    and last_different_nivel_record.timestamp
                ):
                    time_diff = (
                        interaction.timestamp - last_different_nivel_record.timestamp
                    )
            ...
            try:
                ...
                if current_flow is not None:
                    last_different_flow_record = (
                        TelemetryRecord.objects.filter(
                            point_id=point.id, timestamp__lt=interaction.timestamp
                        )
                        .exclude(flow=current_flow)
                        .order_by("-timestamp")
                        .first()
                    )
                else:
                ...
                if last_different_flow_record and last_different_flow_record.timestamp:
                    time_diff = (
                        interaction.timestamp - last_different_flow_record.timestamp
                    )
            ...
            for v_type in variable_types:
                ...
                if v_type in [
                    ...
                    lookup_type = "CAUDAL"
                elif v_type in ["nivel", "water_table", "level"]:
                    lookup_type = "NIVEL"
                elif v_type in ["total", "totalizado"]:
                    lookup_type = "TOTALIZADO"

                days = status_map.get(lookup_type, 0)
                variables_with_status.append(
                    {"type": v_type, "days": days, "is_failing": days > 0}
                )
            ...
            enhanced_interactions.append(
                {
                    ...
                    "veracidad_pct": (
                        round(veracidad_pct, 1) if veracidad_pct is not None else None
                    ),
                    "caudal_probable": (
                        round(caudal_probable, 2)
                        if caudal_probable is not None
                        else None
                    ),
                    ...
                    "last_voucher": (
                        latest_vouchers_map.get(point.id).n_voucher
                        if latest_vouchers_map.get(point.id)
                        else (interaction.n_voucher if interaction.n_voucher else None)
                    ),
                    # Agregar fecha del voucher
                    "last_voucher_date": (
                        latest_vouchers_map.get(point.id).timestamp
                        if latest_vouchers_map.get(point.id)
                        else (interaction.timestamp if interaction.n_voucher else None)
                    ),
                    # Agregar objeto completo del voucher para verificar variables enviadas
                    "last_voucher_obj": (
                        latest_vouchers_map.get(point.id)
                        if latest_vouchers_map.get(point.id)
                        else (interaction if interaction.n_voucher else None)
                    ),
                    "send_dga": (
                        interaction.send_dga
                        if hasattr(interaction, "send_dga")
                        else False
                    ),
        ...
        # ==============================================================================
        if "pct_conectados" not in locals():
            pct_conectados = 0
        if "pct_conectados_count" not in locals():
            pct_conectados_count = 0
        if "pct_conectados_total" not in locals():
            pct_conectados_total = 0

        if "obras_dga_active" not in locals():
            obras_dga_active = 0
        if "num_obras" not in locals():
            num_obras = 0

        # Calcular dga_active_pct si no existe
        if "dga_active_pct" not in locals():
            dga_active_pct = (
                (obras_dga_active / num_obras * 100) if num_obras > 0 else 0
            )
        ...
        if "provider_counts" not in locals():
            provider_counts = {"thethings": 0, "novus": 0, "tdata": 0}
        if "disconnected_gt_1d_count" not in locals():
            disconnected_gt_1d_count = 0
        if "dga_active_status" not in locals():
            dga_active_status = {}
        if "dga_standards_breakdown" not in locals():
            dga_standards_breakdown = []

        if "pct_veracidad" not in locals():
            pct_veracidad = 0
        if "error_page_obj" not in locals():
            error_page_obj = None
        if "pct_cola_dga" not in locals():
            pct_cola_dga = 0
        if "records_in_dga_queue" not in locals():
            records_in_dga_queue = 0
        if "total_registros_cola_dga" not in locals():
            total_registros_cola_dga = 0
        if "notificaciones_pendientes" not in locals():
            notificaciones_pendientes = []
        if "recent_interactions" not in locals():
            recent_interactions = []
        if "enhanced_interactions" not in locals():
            enhanced_interactions = []

        if "use_historical_veracidad" not in locals():
            use_historical_veracidad = False
        if "veracidad_periodo" not in locals():
            veracidad_periodo = []
        if "period_days" not in locals():
            period_days = 30

        if "active_connected_points" not in locals():
            active_connected_points = 0
        if "active_points" not in locals():
            active_points = set()

        # ==============================================================================

        context = {
            "recent_interactions": enhanced_interactions[
                :50
            ...
            "desconectados_count": stats_caudal_probable.get("con_desconexion", 0)
            + stats_caudal_probable.get("desconexiones_parciales", 0),
            "desconectados_parciales": stats_caudal_probable.get(
                "desconexiones_parciales", 0
            ),
            ...
            "pct_veracidad": (
                round(pct_veracidad, 1) if "pct_veracidad" in locals() else 0
            ),  # ✅ FIX: Safety check
            "pct_veracidad_total": (
                veracidad_total_puntos if "veracidad_total_puntos" in locals() else 0
            ),
            "pct_veracidad_con_d5": (
                veracidad_puntos_con_d5 if "veracidad_puntos_con_d5" in locals() else 0
            ),
            "pct_veracidad_count": (
                (veracidad_puntos_con_d5 - veracidad_pasan_probable)
                if "veracidad_puntos_con_d5" in locals()
                and "veracidad_pasan_probable" in locals()
                else 0
            ),
            "pct_veracidad_pasan_probable": (
                veracidad_pasan_probable
                if "veracidad_pasan_probable" in locals()
                else 0
            ),
            "error_page_obj": error_page_obj,
            "pct_cola_dga": round(pct_cola_dga, 1) if "pct_cola_dga" in locals() else 0,
            "pct_cola_dga_count": (
                records_in_dga_queue if "records_in_dga_queue" in locals() else 0
            ),
            "pct_cola_dga_total": (
                total_registros_cola_dga
                if "total_registros_cola_dga" in locals()
                else 0
            ),
            "notificaciones_pendientes": (
                notificaciones_pendientes
                if "notificaciones_pendientes" in locals()
                else []
            ),
            # Listas
            ...
            "use_historical_veracidad": (
                use_historical_veracidad
                if "use_historical_veracidad" in locals()
                else False
            ),
            "veracidad_periodo": (
                veracidad_periodo if "veracidad_periodo" in locals() else []
            ),
            "period_days": period_days if "period_days" in locals() else 30,
            # Stats adicionales
            "stats_caudal_probable": (
                stats_caudal_probable
                if "stats_caudal_probable" in locals()
                else {"con_desconexion": 0}
            ),
            "registro_errores": (
                registro_errores if "registro_errores" in locals() else 0
            ),
            "errores_y_excesos_combinados": (
                errores_y_excesos_combinados
                if "errores_y_excesos_combinados" in locals()
                else []
            ),
            ...
            "variable_filter": (
                variable_filter if "variable_filter" in locals() else "all"
            ),
            # Fecha/hora
            "now": now,
        }
...
def telemetry_monitoring_view(request):
    ...
    for point in points_list:
        ...
        var_types_set = set(variable_types)
        has_caudal_promedio = bool(
            {"caudal_promedio", "flow_avg", "avg_flow"} & var_types_set
        )
        has_caudal = bool({"flow", "caudal"} & var_types_set) or has_caudal_promedio
        has_nivel = bool({"nivel", "water_table", "level"} & var_types_set)
        ...
        # Validar caudal
        flow_safe_monitoring = safe_float(
            last_record.flow if last_record.flow else None
        )
        if has_caudal and flow_safe_monitoring > 0:
            d5_safe_monitoring = safe_float(
                profile.d5 if profile and profile.d5 else None
            )
            if d5_safe_monitoring > 0:
                es_imposible, mensaje = validate_flow_impossible(
                    flow_safe_monitoring, d5_safe_monitoring
                )
                if es_imposible:
                    alerts.append(
                        {
        ...
        if has_nivel and last_record.nivel:
            try:
                nivel_value = safe_float(last_record.nivel)
                if nivel_value > 0:
                    d1_safe_monitoring_level = safe_float(
                        profile.d1 if profile and profile.d1 else None
                    )
                    d3_safe_monitoring_level = safe_float(
                        profile.d3 if profile and profile.d3 else None
                    )
                    es_imposible, mensaje = validate_level_impossible(
                        nivel_value, d1_safe_monitoring_level, d3_safe_monitoring_level
                    )
                    if es_imposible:
                        alerts.append(
                            {
        ...
        try:
            if last_record.timestamp:
                ...
                if last_record.timestamp.tzinfo is None:
                    ...
                    medition_dt = chile_tz.localize(last_record.timestamp)
                else:
                    # Si ya tiene timezone, convertir a chile_tz
                    medition_dt = last_record.timestamp.astimezone(chile_tz)
    ...
    monitoring_data.sort(
        key=lambda x: (len(x["alerts"]), len(x["warnings"]), not x["is_recent"]),
        reverse=True,
    )
    ...
    context = {
        ...
        "total_points": len(
            points_list
        ),  # ✅ FIX: Usar len() en lugar de count() después de convertir a lista
...
def telemetry_monitoring_api(request):
    ...
    data = {"timestamp": now.isoformat(), "points": []}
    ...
    for point in points:
        ...
        # Validar caudal
        flow_safe_api_validation = safe_float(
            flow_value if flow_value is not None else None
        )
        d5_safe_api_validation = safe_float(
            profile.d5 if profile and profile.d5 else None
        )
        if flow_safe_api_validation > 0 and d5_safe_api_validation > 0:
            es_imposible, mensaje = validate_flow_impossible(
                flow_safe_api_validation, d5_safe_api_validation
            )
            if es_imposible:
                alerts.append({"tipo": "Caudal Imposible", "mensaje": mensaje})
        ...
        if last_record.nivel:
            try:
                ...
                if nivel_value > 0:
                    ...
                    if es_imposible:
                        alerts.append({"tipo": "Nivel Imposible", "mensaje": mensaje})
        ...
        data["points"].append(
            {
                ...
                "last_update": (
                    last_record.timestamp.isoformat() if last_record.timestamp else None
                ),
                ...
                "total_diff": safe_float(
                    last_record.total_diff if last_record.total_diff else None
                ),
                "total_today_diff": safe_float(
                    last_record.total_today_diff
                    if last_record.total_today_diff
                    else None
                ),
                "water_table": safe_float(
                    last_record.water_table if last_record.water_table else None
                ),
                "variables": variable_types,
                "has_d6": has_d6,
                "d6_value": d6_value,
...
def telemetry_point_records_api(request, point_id):
    ...
    records = TelemetryRecord.objects.filter(point=point).order_by("-timestamp")[:20]
    ...
    for record in records:
        ...
        if has_caudal_promedio and has_d6:
            try:
                ...
                flow_value = get_interaction_flow_display_data(record).get(
                    "value", flow_value
                )
            except Exception:
                flow_value = record.flow or 0.0
        ...
        records_data.append(
            {
                ...
                "total_diff": safe_float(
                    record.total_diff if record.total_diff else None
                ),
                "nivel": safe_float(record.nivel if record.nivel else None),
                "water_table": safe_float(
                    record.water_table if record.water_table else None
                ),
    ...
    return JsonResponse(
        {"point_id": point_id, "point_name": str(point), "records": records_data}
    )
