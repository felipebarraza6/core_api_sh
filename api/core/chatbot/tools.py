...
def search_points(query, context_client=None):
    ...
    if context_client and len(clean_query) <= 10:
        ...
        if context_points.count() == 1:
            ...
            return [{"id": p.id, "title": p.title, "client": p.project.client.name} for p in context_points[:10]]
    ...
    if client_matches.exists():
        ...
        ).select_related('project', 'project__client', 'owner_user')
        if points.exists():
            return [{"id": p.id, "title": p.title, "client": p.project.client.name if p.project and p.project.client else "N/A"} for p in points[:15]]
    
    # PASO 2: Buscar por términos (Lógica AND / Intersection)
    terms = [t for t in query.split() if len(t) >= 2 and t.lower() not in ('de', 'del', 'la', 'los', 'las', 'el', 'que', 'hay', 'cuales', 'cuantos', 'puntos', 'pozos')]
    ...
    points = points_qs.select_related('project', 'project__client', 'owner_user').distinct()[:15]
    
    return [{"id": p.id, "title": p.title, "client": p.project.client.name if p.project and p.project.client else "N/A"} for p in points]
...
def get_project_measurements(client_name, project_name):
    ...
    for point in points[:10]:
        ...
        if latest:
            actual_date = latest.date_time_last_logger or latest.date_time_medition
            fecha = actual_date.astimezone(chile_tz).strftime("%d/%m %H:%M") if actual_date else "N/A"
            ...
            today_val = safe_float(latest.total_today_diff)
    ...
    res = add_suggestions(res, context="STATS" if "Últimas mediciones" in res else "PROJECT", client=client_name_ctx)
    return res
...
def get_point_latest_data(point_id):
    ...
    try:
        ...
        # Determinar qué variables envía el punto
        variables_enviadas = []
        try:
            for scheme in point.schemes.all():
                for var in scheme.variables.all():
                    v_type = var.get_type_variable_display() or var.type_variable
                    variables_enviadas.append(v_type)
        except: pass
        
        # Formatear fechas
        fecha_sistema = latest.date_time_medition.astimezone(chile_tz).strftime("%d/%m/%Y %H:%M")
        fecha_sensor = latest.date_time_last_logger.astimezone(chile_tz).strftime("%d/%m/%Y %H:%M") if latest.date_time_last_logger else "N/A"
        ...
        if latest.flow:
            ...
            es_promedio = any("promedio" in v.lower() for v in variables_enviadas)
            q_label = "Caudal Promedio" if es_promedio else "Caudal"
            res += f"🌊 *{q_label}:* {latest.flow} L/s\n"
        
        if latest.nivel:
            res += f"📏 *Nivel:* {latest.nivel} mt\n"
            
        if latest.total:
            res += f"🚜 *Totalizador:* {latest.total} m³\n"
            
        if latest.total_today_diff:
            res += f"💧 *Consumo Hoy:* {latest.total_today_diff} m³\n"
        
        res += "\n"
        if latest.days_not_conection > 0:
            res += f"⚠️ *Estado:* Desconectado ({int(latest.days_not_conection)} días)"
        else:
...
def get_client_measurements(client_name):
    ...
    client = clients.first()
    points = CatchmentPoint.objects.filter(project__client=client).select_related('project', 'project__client')
...
def get_point_status_summary():
    ...
    total = CatchmentPoint.objects.filter(data_config_profiles__is_telemetry=True).count()
    ...
    for point in CatchmentPoint.objects.filter(data_config_profiles__is_telemetry=True):
        latest = InteractionDetail.objects.filter(catchment_point=point).order_by('-date_time_medition').first()
        days_val = safe_float(latest.days_not_conection) if latest else 0
        if days_val > 0:
            disconnected += 1
        else:
            connected += 1
...
def get_dga_compliance(client_name):
    ...
    for point in points:
        ...
        has_error = latest.is_error or (latest.return_dga and 'error' in latest.return_dga.lower())
        ...
        if has_voucher:
            ...
            estado_icon = "❌"
            error_msg = latest.return_dga[:50] if latest.return_dga else "Error desconocido"
    ...
    if points_with_dga == 0:
        return f"El cliente '{client.name}' no tiene puntos con cumplimiento DGA activo."
    ...
    for d in details:
        ...
        if 'fecha' in d:
            res += f"   {d['fecha']} | Q:{round(d['flow'], 2)}L/s T:{int(d['total'])}m³\n"
            res += f"   {d['status']}\n"
        else:
            res += f"   {d['status']}\n"
...
def get_point_config(point_name, context_client=None):
    ...
    if data_config:
        ...
        if data_config.d1 > 0: res += f"   • Profundidad Pozo (d1): {data_config.d1} mt\n"
        if data_config.d2 > 0: res += f"   • Posicionamiento Bomba (d2): {data_config.d2} mt\n"
        if data_config.d3 > 0: res += f"   • Posicionamiento Nivel (d3): {data_config.d3} mt\n"
        if data_config.d4 > 0: res += f"   • Diámetro Ducto Salida (d4): {data_config.d4}\"\n"
        if data_config.d5 > 0: res += f"   • Diámetro Flujómetro (d5): {data_config.d5}\"\n"
        if data_config.d6: res += f"   • Lectura Inicial (d6): {data_config.d6} m³\n"
        if data_config.addition > 0: res += f"   • Adición (Reset/Glitch): {data_config.addition} m³\n"
        res += "\n"
        
        # Variables detalladas (Variable model)
        from api.core.models import Variable
        variables = Variable.objects.filter(scheme_catchment__points_catchment=point).select_related('scheme_catchment')
        if variables.exists():
            res += f"📊 *Variables Configuradas*\n"
            for v in variables:
                res += f"   🔹 *{v.label}* ({v.get_type_variable_display() if hasattr(v, 'get_type_variable_display') else v.type_variable})\n"
                res += f"      • Prov: {v.get_service_display() if hasattr(v, 'get_service_display') else v.service} | ID: `{v.str_variable}`\n"
                if v.type_variable == 'TOTALIZADO':
                    res += f"      • Factor Pulsos: {v.pulses_factor}\n"
                if v.type_variable == 'CAUDAL' and v.convert_to_lt:
                    res += f"      • Conv. a Lt: ✅ SI\n"
                if v.type_variable == 'NIVEL' and v.calculate_nivel:
                    res += f"      • Base Cálculo: {v.calculate_nivel}\n"
            res += "\n"
    ...
    if dga_config and dga_config.send_dga:
        ...
        if dga_config.total_granted_dga: res += f"   • Total Anual: {dga_config.total_granted_dga} m³\n"
        res += "\n"
...
def get_client_alerts(client_name):
    ...
    for point in points:
        ...
        try:
            days_disc = float(latest.days_not_conection) if latest.days_not_conection else 0
        except:
            days_disc = 0
        ...
        if latest.is_error:
            errors.append({'point': point.title, 'error': (latest.return_dga[:50] if latest.return_dga else "Error")})
...
def get_point_history(point_name, days=7, context_client=None):
    ...
    for record in records[:10]:
        actual_date = record.date_time_last_logger or record.date_time_medition
        fecha = actual_date.astimezone(chile_tz).strftime("%d/%m %H:%M")
        flow_val = float(record.flow) if record.flow else 0
    ...
    res = add_suggestions(res, context="POINT", point=point.title, client=context_client)
    return res
...
def get_client_errors(client_name, days=7):
    ...
    error_records = InteractionDetail.objects.filter(
        catchment_point__in=points,
        date_time_medition__gte=start_date,
        is_error=True
    ).order_by('-date_time_medition')[:15]
    ...
    for r in error_records:
        fecha = r.date_time_medition.strftime("%d/%m %H:%M")
        res += f"• *{r.catchment_point.title}* ({fecha}):\n  └─ {r.return_dga[:80]}\n\n"
...
def get_global_status():
    ...
    from api.core.models import CatchmentPoint, Client, InteractionDetail, NotificationsCatchment
    ...
    # 1. Resumen de Flota
    total_points = CatchmentPoint.objects.filter(data_config_profiles__is_telemetry=True).count()
    total_clients = Client.objects.count()
    
    disconnected_list = []
    with_errors = 0
    
    points_active = CatchmentPoint.objects.filter(data_config_profiles__is_telemetry=True).select_related('project__client')
    
    for p in points_active:
        latest = InteractionDetail.objects.filter(catchment_point=p).order_by('-date_time_medition').first()
        if latest:
            try:
                days = float(latest.days_not_conection or 0)
                if days > 0:
                    ...
                    vars_status = []
                    try:
                        # Acceder a los esquemas del punto
                        for scheme in p.schemes.all():
                            for var in scheme.variables.all():
                                var_type = var.type_variable
                                has_data = False
                                if var_type == 'TOTALIZADO' and latest.total and float(latest.total) > 0:
                                    has_data = True
                                elif var_type == 'NIVEL' and latest.nivel and float(latest.nivel) > 0:
                                    has_data = True
                                elif var_type in ('CAUDAL', 'CAUDAL_PROMEDIO') and latest.flow and float(latest.flow) > 0:
                                    has_data = True
                                
                                icon = "✅" if has_data else "❌"
                                short_name = "T" if var_type == "TOTALIZADO" else ("N" if var_type == "NIVEL" else "Q")
                                vars_status.append(f"{icon}{short_name}")
                    except: pass
    ...
    notifs = NotificationsCatchment.objects.filter(is_active=True).select_related('point_catchment__project__client').order_by('-id')[:20]
    if notifs.exists():
...
def get_recent_notifications(limit=10):
    ...
    notifs = NotificationsCatchment.objects.filter(
        is_active=True
    ).select_related('point_catchment', 'point_catchment__project__client').order_by('-id')[:limit * 2] # Pedir más para agrupar
...
def compare_points(point1_query, point2_query, context_client=None):
    ...
    if len(p1_results) > 1 or len(p2_results) > 1:
        ...
        if len(p1_results) > 1:
            res += f"Búsqueda 1 ({point1_query}): " + ", ".join([p['title'] for p in p1_results[:3]]) + "\n"
        if len(p2_results) > 1:
            res += f"Búsqueda 2 ({point2_query}): " + ", ".join([p['title'] for p in p2_results[:3]])
        return res

    point1 = CatchmentPoint.objects.get(id=p1_results[0]['id'])
    point2 = CatchmentPoint.objects.get(id=p2_results[0]['id'])
    
    latest1 = InteractionDetail.objects.filter(catchment_point=point1).order_by('-date_time_medition').first()
    latest2 = InteractionDetail.objects.filter(catchment_point=point2).order_by('-date_time_medition').first()
    
    if not latest1 or not latest2:
        return "Uno de los puntos no tiene mediciones recientes para comparar."
    
    def safe_f(val): return float(val) if val else 0.0
    
    res = f"📊 *Comparativa: {point1.title} vs {point2.title}*\n"
    res += f"━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
    
    # Caudal
    q1, q2 = safe_f(latest1.flow), safe_f(latest2.flow)
    diff_q = q1 - q2
    ...
    # Nivel
    n1, n2 = safe_f(latest1.nivel), safe_f(latest2.nivel)
    diff_n = n1 - n2
    res += f"📏 *Nivel (mt):*\n"
    ...
    def get_cons(p):
        rec = InteractionDetail.objects.filter(catchment_point=p, date_time_medition__gte=yesterday).order_by('date_time_medition')
        if rec.count() >= 2:
            return safe_f(rec.last().total) - safe_f(rec.first().total)
        return 0.0
    ...
    res = add_suggestions(res, context="POINT", point=point1.title, client=context_client)
    return res
...
def get_client_stats(client_query, days=7):
    ...
    if not clients.exists(): return f"No encontré al cliente '{client_query}'."
    client = clients.first()
    
    points = CatchmentPoint.objects.filter(project__client=client, data_config_profiles__is_telemetry=True)
    start_date = datetime.now() - timedelta(days=days)
    ...
    stats = InteractionDetail.objects.filter(
        ...
        avg_flow=Avg('flow'),
        max_flow=Max('flow'),
        total_m3=Sum('total_diff') # total_diff suele ser el consumo por registro
    )
    
    if not stats['total_m3']: return f"No hay datos suficientes para generar estadísticas de {client.name}."
...
def get_client_ranking(client_query, metric='CONSUME', days=1):
    ...
    ranking = []
    for p in points:
        data = InteractionDetail.objects.filter(catchment_point=p, date_time_medition__gte=start_date).aggregate(
            val=Sum('total_diff') if metric == 'CONSUME' else Max('flow')
        )
        if data['val']:
...
def get_stuck_points(client_query):
    ...
    stuck = []
    for p in points:
        records = InteractionDetail.objects.filter(catchment_point=p, date_time_medition__gte=yesterday).order_by('date_time_medition')
        if records.count() >= 5:
            first = float(records.first().total or 0)
            last = float(records.last().total or 0)
...
def get_timeseries_analysis(point_name, window_days=7, context_client=None):
    ...
    current_data = InteractionDetail.objects.filter(
        catchment_point_id=point_id,
        date_time_medition__range=(start_date, now)
    ).order_by('date_time_medition')
    
    # 2. Obtener datos anteriores para comparar
    prev_data = InteractionDetail.objects.filter(
        catchment_point_id=point_id,
        date_time_medition__range=(prev_start_date, start_date)
    ).aggregate(
        total_prev=Sum('total_diff'),
        avg_flow_prev=Avg('flow'),
        avg_nivel_prev=Avg('nivel')
    )
    ...
    stats = current_data.aggregate(
        total_curr=Sum('total_diff'),
        avg_flow_curr=Avg('flow'),
        avg_nivel_curr=Avg('nivel'),
        max_flow=Max('flow'),
        min_nivel=Min('nivel')
    )
    ...
    if prev_n > 0:
        ...
        if abs(diff_n) > 0.1:
            status = "⚠️ Bajando" if diff_n > 0.2 else "✅ Subiendo" # En nivel, más metros es más profundo/peor
