"""Nettra 1/hour - UNIFICADO Y MEJORADO"""
...
def run():
    ...
    get_data = CatchmentPoint.objects.filter(
        is_thethings=True, is_tdata=False, is_novus=False, data_config_profiles__is_telemetry=True, frecuency="60"
    )
...
def get_data_nettra(variables, token, point_catchment):
    """Get data by father nettra - UNIFICADO Y MEJORADO"""
        ...
        try:
            if type_variable == "TOTALIZADO":
                ...
                created_register["total"] = total_m3(
                    variable.get("pulses_factor"), value, point_catchment, variable_id=variable.get("id")
                )
    ...
    if best_date_time_last_logger or variable_details:
        ...
        # ✅ FIX: GARANTIZAR REGISTRO CADA HORA - Sin datos nuevos, replicar último válido
        print(f"⚠️ Punto {point_catchment['id']}: Sin datos nuevos de ninguna variable. Replicando último válido.")
        ...
        if last_valid:
            ...
            created_register["date_time_last_logger"] = last_valid.date_time_last_logger.strftime("%Y-%m-%dT%H:%M:%S") if last_valid.date_time_last_logger else None
            created_register["days_not_conection"] = 9999  # Marcar como desconectado
            created_register["is_partial"] = False
            created_register["variable_details"] = []  # Sin detalles
            print(f"   Replicado: Total={last_valid.total}, Nivel={last_valid.nivel}, Flow={last_valid.flow}")
        else:
            # No hay registro previo - crear con valores por defecto
            ...
            print(f"   Sin historial: Creando registro con valores 0")
