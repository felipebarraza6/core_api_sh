...
def run():
    ...
    for data in serializer.data:
        try:
            ...
            get_data_novus(variables, token, point_catchment)
...
def get_data_novus(variables, token, point_catchment):
    ...
    for variable in variables:
        ...
        if variable.get("token_service"):
            ...
            data = get_data_with_retry(
                get_data_tago, token, variable.get("str_variable")
            )
    ...
    try:
        ...
        print(f"❌ ERROR PUNTO {point_catchment['id']}: Error guardando registro en BD: {e}")
