"""
Twin 1/hour - MIGRADO AL SISTEMA DINÁMICO DE PROVEEDORES
...
def run():
    ...
    for data in serializer.data:
        ...
        try:
            ...
            get_data_twin_dynamic(variables, token, point_catchment)
        except KeyError as e:
            print(f"Missing key in data dictionary: {e}")


def get_data_with_retry_dynamic(point_id, provider_name, variable_name, max_retries=3, backoff_factor=2):
    """
    Retry inteligente con backoff exponencial - Versión dinámica

    Reemplaza: get_data_with_retry(getter_func, token, variable_name)
    Por: get_data_with_retry_dynamic(point_id, provider_name, variable_name)
    """
    manager = get_provider_manager()

    for attempt in range(max_retries):
        try:
            # ✅ NUEVO: Obtener la mejor configuración para este punto y variable
            provider_config = manager.get_best_provider_for_point(point_id, variable_name)
            ...
            if attempt == max_retries - 1:
                print(f"❌ Error después de {max_retries} intentos con {provider_name}: {e}")
                return None
...
def get_data_twin_dynamic(variables, token, point_catchment):
    """
    Get data by father twin - VERSIÓN DINÁMICA
