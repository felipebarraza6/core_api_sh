"""Twin 10/minutos - UNIFICADO Y MEJORADO"""
...
def run():
    """Punto de inicio de la ejecución frecuencia 10/min"""
    get_data = CatchmentPoint.objects.filter(
        is_tdata=True, is_thethings=False, is_novus=False, data_config_profiles__is_telemetry=True, frecuency="10"
    )
...
def get_data_twin(variables, token, point_catchment):
    ...
    # Validation of frequency enabled
    ...
    for variable in variables:
        ...
        if data is None:
            print(f"⚠️ NO DATA FETCHED for {variable.get('str_variable')} - Using 0 as fallback")
            ...
            log_variable_processing(
                    ...
                    False,
                    "No se pudo obtener datos (Fetch Failed)"
                )
