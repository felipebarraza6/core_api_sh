"""Twin 5/minutos - UNIFICADO Y MEJORADO"""
...
def run():
    """Punto de inicio de la ejecución frecuencia 5/min"""
    get_data = CatchmentPoint.objects.filter(
        is_tdata=True, is_thethings=False, is_novus=False, data_config_profiles__is_telemetry=True, frecuency="5"
    )
...
def get_data_twin(variables, token, point_catchment):
    ...
    for variable in variables:
        ...
        if created_register.get("date_time_last_logger"):
            ...
            if best_date_time_last_logger is None or days_not_conection < (datetime.now(chile).replace(minute=0, second=0, microsecond=0) - best_date_time_last_logger.astimezone(chile).replace(minute=0, second=0, microsecond=0)).days:
                # Simplificación: si days_not_conection es mejor que el actual, actualizamos heartbeat
                # Pero para no complicar con deltas, simplemente usamos el primero que llegue que sea 0, o el mejor.
    ...
    # Usamos la mejor fecha disponible para el registro, pero los peores días para el estado
    if best_date_time_last_logger or variable_details:
        if not best_date_time_last_logger and variable_details:
            # Fallback si ninguna tiene timestamp válido pero hay variables
            best_date_time_last_logger = variable_details[0].get("timestamp")
