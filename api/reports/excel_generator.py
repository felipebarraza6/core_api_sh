...
def generate_excel_by_project(points: List[CatchmentPoint], project_name: Optional[str] = None) -> BytesIO:
    ...
    for point in points:
        ...
        for record in records:
            ...
            if record.timestamp:
                medition_date_chile = record.timestamp.astimezone(chile_tz)
                ws.cell(row, 2, medition_date_chile.strftime('%Y-%m-%d %H:%M:%S'))
            else:
                ws.cell(row, 2, 'Sin registros')
            # Mejorar formato de Fecha Logger
            if record.metadata__last_logger_timestamp:
                logger_date_chile = record.metadata__last_logger_timestamp.astimezone(chile_tz)
...
def generate_excel_by_point(point: CatchmentPoint, year: Optional[int] = None, month: Optional[int] = None) -> BytesIO:
    ...
    if not TelemetryRecord.objects.filter(
        point=point,
    ...
    if dga_records.exists():
        ...
        for idx, dga_rec in enumerate(dga_records_list):
            ...
            # Convertir fechas a zona horaria de Chile antes de formatear
            if dga_rec.timestamp:
                medition_date_chile = dga_rec.timestamp.astimezone(chile_tz)
                ws_summary.cell(row, 2, medition_date_chile.strftime('%Y-%m-%d %H:%M:%S'))
            else:
                ws_summary.cell(row, 2, 'Sin registros')
            # Mejorar formato de Fecha Logger
            if dga_rec.metadata__last_logger_timestamp:
                logger_date_chile = dga_rec.metadata__last_logger_timestamp.astimezone(chile_tz)
                logger_date = logger_date_chile.strftime('%Y-%m-%d %H:%M:%S')
            else:
                logger_date = 'Sin registros'
            ...
            if has_caudal_promedio:
                ...
                if idx < len(dga_records_list) - 1:
                    ...
                    prev_record = TelemetryRecord.objects.filter(
                        point=point,
                        timestamp__lt=dga_rec.timestamp
    ...
    annual_stats = TelemetryRecord.objects.filter(
        point=point,
        timestamp__gte=year_start,
        timestamp__lte=year_end
    ).aggregate(
    ...
    primer_registro_anual = TelemetryRecord.objects.filter(
        point=point,
        timestamp__gte=year_start,
        timestamp__lte=year_end
    ).order_by('timestamp').first()
    ...
    if min_flow and min_flow <= 0:
        min_flow_positive = TelemetryRecord.objects.filter(
            point=point,
            timestamp__gte=year_start,
            timestamp__lte=year_end,
            flow__gt=0
    ...
    if primer_registro_anual:
        fecha_primer = primer_registro_anual.timestamp.strftime('%Y-%m-%d %H:%M:%S') if primer_registro_anual.timestamp else 'Sin registros'
    ...
    for month_key in sorted(months_data.keys()):
        ...
        month_dga_records = list(TelemetryRecord.objects.filter(
            point=point,
        ...
        # Filtra solo registros con hora exacta (:00:00) en timestamp
        ...
        if has_caudal_promedio and month_records:
            ...
            first_record_ts = month_records[0].timestamp
            # Cargar el registro anterior más cercano (solo 1 query)
            prev_record_cache = TelemetryRecord.objects.filter(
                point=point,
                timestamp__lt=first_record_ts
            ).order_by('-timestamp').only('id', 'timestamp', 'metadata__last_logger_timestamp', 'total').first()
...
def generate_excel_last_month_by_points(points: List[CatchmentPoint], project_name: Optional[str] = None) -> BytesIO:
    ...
    for point in points:
        ...
        month_records = TelemetryRecord.objects.filter(
            point=point,
            timestamp__gte=month_start,
            timestamp__lte=month_end
        ).order_by('timestamp')
        ...
        # Total actual
        last_record = month_records.order_by('-timestamp').first()
        ...
        annual_stats = TelemetryRecord.objects.filter(
            point=point,
            timestamp__gte=year_start_annual,
            timestamp__lte=year_end_annual
        ).aggregate(
    ...
    for point in points:
        ...
        if dga_config and dga_config.send_dga and dga_config.flow_granted_dga:
            ...
            veces_supero_autorizado = TelemetryRecord.objects.filter(
                point=point,
                timestamp__gte=month_start,
                timestamp__lte=month_end,
        ...
        if has_caudal_promedio and month_records_list:
            first_record_ts = month_records_list[0].timestamp
            prev_record_cache = TelemetryRecord.objects.filter(
                point=point,
                timestamp__lt=first_record_ts
            ).order_by('-timestamp').only('id', 'timestamp', 'metadata__last_logger_timestamp', 'total').first()
...
def generate_excel_last_year_by_points(points: List[CatchmentPoint], project_name: Optional[str] = None) -> BytesIO:
    ...
    for analysis in all_analysis:
        ...
        year_records_iter = TelemetryRecord.objects.filter(
            point=point,
            timestamp__gte=year_start,
            timestamp__lte=year_end
        ).order_by('timestamp').iterator(chunk_size=2000)
        ...
        for record in year_records_iter:
            ...
            if record.metadata__last_logger_timestamp:
                if record.metadata__last_logger_timestamp.tzinfo is None:
                    logger_date_utc = pytz.utc.localize(record.metadata__last_logger_timestamp)
            ...
            # Fecha UTC
            ws.cell(row, 1, record.timestamp.strftime('%Y-%m-%d %H:%M:%S') if record.timestamp else '-')
            
            # Fecha Chile
            if record.timestamp:
                if record.timestamp.tzinfo is None:
                    medition_date_utc = pytz.utc.localize(record.timestamp)
...
def generate_excel_annual_compressed(points: List[CatchmentPoint], project_name: Optional[str] = None) -> BytesIO:
    ...
    for point in points:
        ...
        monthly_data = TelemetryRecord.objects.filter(
            point=point,
