...
import pytz
from django.db.models import Avg, Count, F, FloatField, IntegerField, Max, Min, Sum
...
def calculate_days_without_data(point_id: int) -> Tuple[int, Optional[datetime]]:
    ...
    last_record = (
        TelemetryRecord.objects.filter(point_id=point_id).order_by("-timestamp").first()
    )

    if not last_record or not last_record.timestamp:
        return -1, None
...
def get_last_correct_values(point_id: int) -> Dict:
    ...
    last_valid = (
        TelemetryRecord.objects.filter(point_id=point_id, data__total__isnull=False)
        .exclude(data__total="")
        .order_by("-timestamp")
        .first()
    )
    ...
    result = {
        "fecha": (
            last_valid.timestamp.strftime("%Y-%m-%d %H:%M")
            if last_valid.timestamp
            else "N/A"
        ),
        "valores": {},
    }
    ...
    for var in variables:
        if var.internal_code == "total" and last_valid.total:
            pf = var.scale_factor if var.scale_factor else 1
            result["valores"][f"VolumenAcum (factor {pf})"] = int(
                float(last_valid.total)
            )
        elif (
            var.internal_code in ["nivel", "water_table", "level"] and last_valid.nivel
        ):
            result["valores"]["Nivel (base 1)"] = float(last_valid.nivel)
        elif var.internal_code in ["flow", "caudal"] and last_valid.flow:
...
def detect_anomalies_optimized(point_id: int, months_back: int = 6) -> List[Dict]:
    ...
    for notif in reset_notifications:
        ...
        nearby_rec = (
            TelemetryRecord.objects.filter(
                point_id=point_id, timestamp__lte=notif.start_date
            )
            .order_by("-timestamp")
            .first()
        )
        ...
        anomalies.append(
            {
                ...
                "fecha": (
                    notif.start_date.strftime("%Y-%m-%d") if notif.start_date else "N/A"
                ),
                "descripcion": (
                    notif.message[:80] if notif.message else "Reset detectado"
                )
                + vals_str,
            }
        )
    ...
    daily_counts = (
        TelemetryRecord.objects.filter(point_id=point_id, timestamp__gte=start_date)
    ...
    for dc in daily_counts_list:
        if prev_day and dc["dia"]:
            ...
            if gap > 1:
                ...
                last_rec = (
                    TelemetryRecord.objects.filter(
                        point_id=point_id, timestamp__lt=dc["dia"]
                    )
                    .order_by("-timestamp")
                    .first()
                )
    ...
    if profile:
        ...
        if profile.d5:
            ...
            impossible_flows = TelemetryRecord.objects.filter(
                point_id=point_id, timestamp__gte=start_date, data__flow__gt=max_flow
            ).count()

            if impossible_flows > 0:
                # Obtener un ejemplo
                bad_rec = TelemetryRecord.objects.filter(
                    point_id=point_id,
                    timestamp__gte=start_date,
                    data__flow__gt=max_flow,
                ).first()
                vals_str = ""
                if bad_rec:
        ...
        # Nivel imposible
        if profile.d1:
            impossible_levels = TelemetryRecord.objects.filter(
                point_id=point_id,
                timestamp__gte=start_date,
                data__nivel__gt=float(profile.d1),
            ).count()
    ...
    sample_records = list(
        TelemetryRecord.objects.filter(
            point_id=point_id, timestamp__gte=start_date, data__total__isnull=False
        )
        .order_by("timestamp")
        .values("timestamp", "data")[:5000:10]
    )  # cada 10vo registro
    ...
    if len(sample_records) > 2:
        ...
        for rec in sample_records:
            current_total = rec["data"].get("total")
            if prev_total is not None and current_total == prev_total:
                if stagnant_start is None:
                    stagnant_start = rec["timestamp"]
            else:
                if stagnant_start and rec["timestamp"]:
                    hours_stagnant = (
                        rec["timestamp"] - stagnant_start
                    ).total_seconds() / 3600
                    if hours_stagnant > 24:
                        # Buscar pulsos en ese momento
                        st_rec = TelemetryRecord.objects.filter(
                            point_id=point_id, timestamp=stagnant_start
                        ).first()
                        p = (
                            int(st_rec.pulses)
                            if st_rec and st_rec.pulses is not None
                            else "N/A"
                        )
    ...
    anomalies.sort(
        key=lambda x: (severity_order.get(x["severidad"], 3), x["fecha"]), reverse=True
    )
...
def get_urgency_level(
    days_without_data: int, critical_anomalies: int
) -> Tuple[str, str]:
...
def generate_ot_soporte_pdf(
    point_id: int, motivo: str = None, contacto_cliente: str = None
) -> BytesIO:
    """
    ...
    try:
        point = CatchmentPoint.objects.select_related("project__client").get(
            id=point_id
        )
    ...
    try:
        logo_url = (
            "https://smarthydro.cl/wp-content/uploads/2023/12/SmartHydro-Logo.png"
        )
    ...
    # Variables del punto
    vars_list = ", ".join([v.name for v in variables]) or "Sin variables"
    ...
    header_data = [
        ...
        [
            "IMEI/Token:",
            (
                profile.token_service[:50] + "..."
                if profile and profile.token_service and len(profile.token_service) > 50
                else (profile.token_service if profile else "N/A")
            ),
        ],
        [
            "Ubicación:",
            f"{point.lat}, {point.lon}" if point.lat and point.lon else "Pendiente",
        ],
        ["Variables:", vars_list],
    ]
    ...
    elements.append(
        Paragraph("<b>2. Descripción del problema o solicitud</b>", section_style)
    )
    ...
    if motivo:
        ...
        elements.append(
            Paragraph(
                f"    1. Punto en uso, pero sin conexión desde hace {days_without_data} días.",
                normal_style,
            )
        )
    else:
        elements.append(
            Paragraph("    1. Revisión preventiva solicitada.", normal_style)
        )
    ...
    if days_without_data >= 0:
        elements.append(
            Paragraph(
                f"○ El punto se encuentra sin datos nuevos desde hace <b>{days_without_data} días</b>",
                normal_style,
            )
        )
    else:
        elements.append(
            Paragraph("○ El punto no tiene registros históricos", normal_style)
        )

    if last_values and last_values.get("valores"):
        elements.append(
            Paragraph(
                f"○ <b>Últimos valores correctos</b> ({last_values.get('fecha', 'N/A')}):",
                normal_style,
            )
        )
    ...
    elements.append(
        Paragraph(
            "<b>○ Estado de Pulsos y Totalizado (últimos 6 meses):</b>", normal_style
        )
    )
    ...
    pulse_stats = (
        TelemetryRecord.objects.filter(
            point_id=point_id, timestamp__gte=six_months_ago, data__pulses__isnull=False
        )
        .annotate(val_pulses=Cast("data__pulses", IntegerField()))
        .aggregate(
            total_records=Count("id"),
            min_pulses=Min("val_pulses"),
            max_pulses=Max("val_pulses"),
            avg_pulses=Avg("val_pulses"),
        )
    )

    total_stats = (
        TelemetryRecord.objects.filter(
            point_id=point_id, timestamp__gte=six_months_ago, data__total__isnull=False
        )
        .exclude(data__total="")
        .annotate(val_total=Cast("data__total", FloatField()))
        .aggregate(min_total=Min("val_total"), max_total=Max("val_total"))
    )
    ...
    consumo_periodo = (
        TelemetryRecord.objects.filter(
            ...
            data__total_diff__isnull=False,
        )
        .annotate(val_total_diff=Cast("data__total_diff", FloatField()))
        .aggregate(sum_diff=Sum("val_total_diff"))["sum_diff"]
        or 0
    )
    ...
    if anomalies:
        elements.append(
            Paragraph(
                "<b>○ Anomalías detectadas en historial (últimos 6 meses):</b>",
                normal_style,
            )
        )
        elements.append(Spacer(1, 0.05 * inch))
        ...
        for idx, a in enumerate(anomalies[:10]):  # Máximo 10
            ...
            icon = (
                ...
                if a["tipo"] == "RESET"
                    ...
                    "📉"
                    if "GAP" in a["tipo"]
                    else ("⚠️" if "ESTANCADO" in a["tipo"] else "❌")
                )
            )
        ...
        anomaly_table = Table(
            anomaly_data,
            colWidths=[0.3 * inch, 1.1 * inch, 0.9 * inch, 0.9 * inch, 3.5 * inch],
        )
        ...
        for idx, row_color in enumerate(row_colors):
            ...
            if anomalies[idx]["severidad"] == "CRÍTICA":
                ...
                table_style.append(
                    ("FONTNAME", (2, idx + 1), (2, idx + 1), "Helvetica-Bold")
                )
    ...
    elements.append(
        Paragraph("<b>4. Especificaciones del trabajo solicitado</b>", section_style)
    )
    ...
    urgency_msg = (
        f"Punto sin datos hace {days_without_data} días"
        if days_without_data > 0
        else "Revisión preventiva"
    )
    if critical_count > 0:
    ...
    elements.append(
        Paragraph(
            f"○ <b>Nivel de urgencia:</b> {urgency} - {urgency_msg}", normal_style
        )
    )
    elements.append(
        Paragraph("○ <b>Plazo requerido:</b> Definido por Hardware.", normal_style)
    )
    elements.append(Spacer(1, 0.15 * inch))
    ...
    client_email = contacto_cliente or (
        point.project.client.contact_email
        if point.project
        and point.project.client
        and hasattr(point.project.client, "contact_email")
        else "Sin detalle"
    )
    elements.append(
        Paragraph(
            f"○ <b>Persona de contacto del cliente:</b> {client_email}", normal_style
        )
    )
    elements.append(
        Paragraph("○ <b>Contacto interno (soporte):</b> Sin detalle.", normal_style)
    )
    elements.append(Spacer(1, 0.15 * inch))
    ...
    elements.append(
        Paragraph("○ <b>Autorización interna:</b> Pendiente.", normal_style)
    )
    elements.append(
        Paragraph(
            "○ <b>Aprobación del cliente (si aplica):</b> Pendiente.", normal_style
        )
    )
