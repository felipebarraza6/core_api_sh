...
def generate_telemetry_analysis_pdf(points: List, project_name: Optional[str] = None, user_info: Optional[str] = None) -> BytesIO:
    ...
    for point_data in points:
        ...
        if analisis.get('telemetria_dia'):
            ...
            yesterday_records = TelemetryRecord.objects.filter(
                point_id=point_id,
            ...
            if yesterday_records.exists():
                ...
                for record in yesterday_records:
                    ...
                    diff_seg = 0
                    if record.timestamp and record.date_time_last_logger:
                        try:
                            if record.timestamp.tzinfo is None:
                                dt_med = chile_tz.localize(record.timestamp)
                            else:
                                dt_med = record.timestamp
        ...
        has_caudal = variables.filter(internal_code__in=["flow", "caudal"]).exists()
        has_nivel = variables.filter(
            internal_code__in=["nivel", "water_table", "level"]
        ).exists()
        ...
        chart_records = TelemetryRecord.objects.filter(
            point_id=point_id,
            timestamp__gte=periodo_inicio,
            timestamp__lte=periodo_fin
        ).order_by('timestamp')
        ...
        try:
            ...
            dga_records = TelemetryRecord.objects.none()
