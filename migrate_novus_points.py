#!/usr/bin/env python3
"""
Migrar puntos Novus al sistema dinámico de proveedores

Este script configura automáticamente los puntos que tienen is_novus=True
...
def migrate_novus_points():
    """Migrar puntos novus al sistema dinámico"""

    print("🔄 MIGRACIÓN: Puntos Novus → Sistema Dinámico")
    print("=" * 50)
    ...
    # Obtener puntos novus
    novus_points = CatchmentPoint.objects.filter(is_novus=True).prefetch_related(
        'schemes__variables'
    )

    print(f"📊 Encontrados {novus_points.count()} puntos con is_novus=True")
    ...
    for point in novus_points:
        ...
        try:
            profile_configs = point.data_config_profiles.all()
            if not profile_configs.exists():
                print("   ⚠️ No tiene configuraciones de perfil")
                skipped_count += 1
                continue

            # Obtener las variables de todos los perfiles
            variables_by_service = {}
            for profile in profile_configs:
                if hasattr(profile, 'schemes') and profile.schemes.exists():
                    for scheme in profile.schemes.all():
                        if hasattr(scheme, 'variables'):
                            for var in scheme.variables:
                                service = var.get('service', 'NETTRA')  # Default fallback
            ...
            for service, variables in variables_by_service.items():
                ...
                point_code = first_var.get('token_service', f"point_{point.id}")
                    ...
                    config = manager.create_provider_config(
                        ...
                        config={
                            ...
                            'device_config': {
                                ...
                                'variable_names': [v.get('str_variable') for v in variables]
                            },
    ...
    print(f"📈 Total procesados: {novus_points.count()}")
    ...
    if migrated_count > 0:
        ...
        print("from .novus_dynamic import run as run_novus_dynamic")

    return migrated_count


def test_dynamic_novus():
    """Probar el sistema dinámico con un punto novus"""

    print("\n🧪 TESTING: Sistema Dinámico Novus")
    print("=" * 50)

    # Obtener un punto novus migrado
    points_with_config = CatchmentPointProvider.objects.filter(
        point__is_novus=True
    ).values_list('point_id', flat=True).distinct()

    if not points_with_config:
        print("❌ No hay puntos novus migrados para probar")
        return
    ...
    try:
        ...
        result = get_data_with_retry_dynamic(
            point_id=point_id,
            provider_name='nettra',  # Probar con nettra
...
if __name__ == "__main__":
    ...
    print("\n🎯 PRÓXIMOS PASOS:")
    print("1. Probar el cronjob dinámico: python3 api/cronjobs/telemetry/novus_dynamic.py")
    print("2. Si funciona, reemplazar novus.py con novus_dynamic.py")
    print("3. Repetir proceso con otros cronjobs (twin.py, nettra.py, etc.)")
    print("4. Una vez probado, eliminar archivos getters/")